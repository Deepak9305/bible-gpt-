...
export default function Layout() {
  ...
  return (
    <div className={`min-h-screen flex flex-col ${theme === 'dark' ? 'bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      ...
      {/* Mobile Bottom Nav */}
      <nav className={`fixed bottom-0 left-0 right-0 border-t md:hidden backdrop-blur-lg ${
        theme === 'dark' 
        ...
        <div className="flex justify-around items-center h-16 px-2">
          {navItems.map(({ to, icon: Icon, label }) => (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                `flex flex-col items-center justify-center w-full h-full space-y-1 rounded-xl transition-all duration-200 ${
                  isActive
